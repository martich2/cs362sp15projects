...
int adventureCard(struct gameState *state, int currentPlayer, int cardDrawn, int temphand[MAX_HAND], int drawntreasure, int z) {
	...
	if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to decki
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
    adventureCard(state, currentPlayer, cardDrawn, temphand[MAX_HAND], drawntreasure, z);
      return 0;
    ...
    case feast:
      feastCard(state,temphand,currentPlayer,choice1);
      return 0;
    ...
    case mine:
        mineCard(state, choice1, choice2, currentPlayer, handPos);
        return 0;
    case remodel:
